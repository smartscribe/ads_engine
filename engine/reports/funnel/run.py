"""Signup funnel report orchestrator.

Runs the full pipeline: pull → compute → qc → render.

Usage:
    python3 -m engine.reports.funnel.run               # full pipeline
    python3 -m engine.reports.funnel.run --no-pull     # use cached raw/
    python3 -m engine.reports.funnel.run --stage qc    # only QC (assumes model.json exists)
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Signup funnel report pipeline")
