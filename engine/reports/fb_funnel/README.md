...
python3 -m engine.reports.fb_funnel.run

# Render-only from cached raw data (instant, for iterating on the HTML):
python3 -m engine.reports.fb_funnel.run --no-pull

# Just one stage:
python3 -m engine.reports.fb_funnel.run --stage qc
```
...
| `pull.py` | Four Metabase queries → raw JSON in `data/performance/snapshots/fb_funnel/raw/` |
...
Re-run weekly, or whenever Chris asks for an updated FB forms number. The pull takes ~2s. Cached raw data is safe to ship around — it's a point-in-time snapshot.
